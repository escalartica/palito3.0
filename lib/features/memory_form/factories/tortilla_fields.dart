...
class TortillaFields implements DynamicFieldGenerator {
    ...
    return [
      ...
      _buildCompactChipGroup(
        ...
        [
          ...
          {'label': 'Lerele', 'icon': Icons.auto_awesome_rounded},
        ],
        data,
        onUpdate,
        isMulti: false,
  ...
  Widget _buildCompactChipGroup(
    ...
    if (hasCustomOption) {
      if (isMulti) {
        ...
        isCustomSelected = selectedItems.contains('Otro');
      } else {
        isCustomSelected = data[key] == 'Otro';
      }
    }
                    ...
                    onTap: () {
                      ...
                      if (option == 'Otro') {
                        if (isSelected) {
                          ...
                          final List currentItems = data[key] is List
                              ? List.from(data[key])
                              : [];

                          currentItems.remove('Otro');

                          onUpdate(key, currentItems);
                        }
                      }
                    },
                  );
    ...
    return TweenAnimationBuilder<double>(
      ...
      child: Container(
        ...
        child: TextFormField(
          ...
          onChanged: (value) {
            _updateCustomIngredient(value, data, onUpdate);
          },
        ),
      ),
    );
  ...
  void _updateCustomIngredient(
    ...
    final List selectedItems = data['ingredientes'] is List
        ? List.from(data['ingredientes'])
        : [];
    ...
    final String customValue = value.trim();

    if (customValue.isNotEmpty) {
      selectedItems.add(customValue);
    }

    onUpdate('ingredientes', selectedItems);
  }
