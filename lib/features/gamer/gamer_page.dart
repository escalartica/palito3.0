...
class _GamerPageState extends ConsumerState<GamerPage>
  ...
  List<Map<String, dynamic>> _players = [
    {'name': 'CeH',        'icon': Icons.person_rounded,    'color': _kYellow,              'points': 0, 'medals': 0},
    {'name': 'Eme',        'icon': Icons.favorite_rounded,  'color': _kRed,                 'points': 0, 'medals': 0},
    {'name': '🤖 Palito App', 'icon': Icons.smart_toy_rounded, 'color': _kDark,             'points': 0, 'medals': 0},
  ];
  ...
  final List<Map<String, dynamic>> _achievements = [
    {'id': 'king_flavor',  'title': 'Rey del Sabor',     'desc': 'Alcanzar los 40 puntos o más en la sesión.',        'icon': Icons.workspace_premium_rounded,        'color': Colors.amber,      'unlocked': false},
    {'id': 'spicy_streak', 'title': 'Racha Picante',     'desc': 'Acumular una racha de más de 10 decisiones.',       'icon': Icons.local_fire_department_rounded,    'color': Colors.deepOrange, 'unlocked': false},
    {'id': 'soul_table',   'title': 'Alma de la Mesa',   'desc': 'Interactuar con todos los comensales.',             'icon': Icons.groups_rounded,                   'color': Colors.purple,     'unlocked': false},
  ];
  ...
  void initState() {
    ...
    _winnerScaleController = AnimationController(vsync: this, duration: const Duration(milliseconds: 400));
    _winnerScaleAnimation  = CurvedAnimation(parent: _winnerScaleController, curve: Curves.elasticOut);
  ...
  Future<void> _loadPersistedData() async {
    try {
      ...
      if (savedPlayersJson != null) {
        ...
        if (decoded is List) {
          final restored = decoded.whereType<Map>().map<Map<String, dynamic>>((item) => {
            'name':   item['name']?.toString() ?? 'Comensal',
            'icon':   _getIconData(item['iconCode'] is int ? item['iconCode'] as int : 0),
            'color':  Color(item['colorValue'] is int ? item['colorValue'] as int : 0xFF9E9E9E),
      ...
      final maxPts = _players.fold<int>(0, (m, p) => (p['points'] as int? ?? 0) > m ? p['points'] as int : m);
  ...
  Future<void> _savePersistedData() async {
    try {
      ...
      final serialized = _players.map((p) {
        final icon  = p['icon']  is IconData ? p['icon']  as IconData : Icons.face_rounded;
  ...
  Future<void> _syncGamerStats() async {
    ...
    final totalScore = _players.fold<int>(0, (s, p) => s + (p['points'] as int? ?? 0));
  ...
  void _checkAndUnlockAchievements(int maxPoints, int streak, {bool fromLoad = false}) {
    ...
    WidgetsBinding.instance.addPostFrameCallback((_) {
      ...
      _showAchievementUnlockedDialog(
        a['title'] as String, a['desc'] as String,
        a['icon']  as IconData, a['color'] as Color,
      );
    });
  }
  ...
  void _resetSessionScores() {
    setState(() {
      for (final p in _players) { p['points'] = 0; p['medals'] = 0; }
      _history.clear();
      ...
      _highlightedIndex = -1;
      for (final a in _achievements) { a['unlocked'] = false; }
    });
    _savePersistedData();
    HapticFeedback.mediumImpact();
  ...
  void _clearHistory() {
    ...
    setState(() { _history.clear(); _punishmentCounts.clear(); });
    _savePersistedData();
    HapticFeedback.mediumImpact();
  ...
  void _resolveChallengeResult(String playerName, bool succeeded) {
    ...
    _showFeedbackSnackbar(
      succeeded ? '🎉 ¡+15 puntos para $playerName!' : '❌ Reto no superado por $playerName',
    );
  }
        ...
        child: SafeArea(
          child: Column(
            ...
            children: [
              ...
              const SizedBox(height: 20),
              Text('🎯 Evaluar Juicio de $playerName',
                  style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: _kDark),
                  textAlign: TextAlign.center),
              ...
              Container(
                ...
                child: Text(challengeText,
                    style: GoogleFonts.inter(fontSize: 14, color: _kDark, fontWeight: FontWeight.bold),
                    textAlign: TextAlign.center),
              ),
              const SizedBox(height: 24),
              Row(children: [
                Expanded(child: ElevatedButton(
                  onPressed: () { _resolveChallengeResult(playerName, false); Navigator.pop(ctx); },
                  style: ElevatedButton.styleFrom(
                ...
                Expanded(child: ElevatedButton(
                  onPressed: () { _resolveChallengeResult(playerName, true); Navigator.pop(ctx); },
                  style: ElevatedButton.styleFrom(
                    backgroundColor: const Color(0xFF10B981), foregroundColor: Colors.white, elevation: 0,
                    shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(16)),
                    padding: const EdgeInsets.symmetric(vertical: 14),
                  ),
        ...
        child: Column(
          ...
          children: [
            Row(
              ...
              children: [
                Text('Añadir Juicio Picante',
                    style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold, color: _kDark)),
            ...
            SizedBox(
              ...
              child: ElevatedButton(
                ...
                onPressed: () {
                  ...
                  setState(() { _palitoChallenges.add(challenge); _currentChallenge = challenge; });
                ...
                child: Text('Guardar y Aplicar Juicio',
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(child: Row(children: [
                    ...
                    Expanded(child: Text('Insignias y Logros de la Mesa',
                        style: GoogleFonts.outfit(fontSize: 22, fontWeight: FontWeight.bold, color: _kDark))),
                  ])),
                  IconButton(icon: const Icon(Icons.close_rounded), onPressed: () => Navigator.pop(ctx)),
                ],
              ...
              const SizedBox(height: 8),
              Text('Ranking ordenado por puntuación, medallas e insignias desbloqueadas.',
                  style: GoogleFonts.inter(fontSize: 13, color: Colors.grey.shade600)),
              const SizedBox(height: 16),
              ...sorted.map((player) {
                    ...
                    children: [
                      Expanded(child: Row(children: [
                        CircleAvatar(
                          ...
                          child: Icon(player['icon'] as IconData, color: playerColor, size: 18),
                        ),
                        const SizedBox(width: 12),
                        Expanded(child: Text(player['name'] as String,
                      ...
                      Column(
                        ...
                        children: [
                          _BadgePill(text: '🏆 ${player['medals']} medallas', bgColor: _kYellow.withValues(alpha: 0.25)),
                          const SizedBox(height: 6),
                          _BadgePill(text: '⭐ ${player['points']} pts', bgColor: _kRed.withValues(alpha: 0.15), textColor: _kRed),
                        ],
                      ),
                    ],
                  ),
              ...
              const SizedBox(height: 16),
              Text('Logros Especiales Globales',
              ...
              ..._achievements.map((a) {
                ...
                return Container(
                  ...
                  decoration: BoxDecoration(
                    ...
                    border: Border.all(color: unlocked ? color.withValues(alpha: 0.3) : Colors.grey.shade300),
                  ),
                  child: Row(children: [
                    Icon(a['icon'] as IconData, color: unlocked ? color : Colors.grey.shade400, size: 28),
                    const SizedBox(width: 12),
                    ...
                    Icon(unlocked ? Icons.check_circle_rounded : Icons.lock_rounded,
  ...
  void _showZonaGamerProModal() {
    final totalPts        = _players.fold<int>(0, (s, p) => s + (p['points'] as int? ?? 0));
        ...
        child: SingleChildScrollView(
          child: Column(
            ...
            children: [
              Row(
                ...
                children: [
                  Expanded(child: Row(children: [
                    ...
                    const SizedBox(width: 10),
                    Expanded(child: Text('Panel Pro de Zona Gamer',
                        style: GoogleFonts.outfit(fontSize: 20, fontWeight: FontWeight.bold, color: _kDark))),
                  ])),
                  IconButton(icon: const Icon(Icons.close_rounded), onPressed: () => Navigator.pop(ctx)),
                ],
              ...
              const SizedBox(height: 12),
              Text('Estadísticas globales en tiempo real y rendimiento analítico de la sesión en Palito.',
                  style: GoogleFonts.inter(fontSize: 13, color: Colors.grey.shade600)),
              const SizedBox(height: 20),
              Container(
                padding: const EdgeInsets.all(16),
                ...
                child: Row(
                  ...
                  children: [
                    _ProStat(label: '👥 Comensales', value: '${_players.length}'),
                    Container(height: 30, width: 1, color: Colors.grey.shade300),
                    _ProStat(label: '⚡ Decisiones',  value: '$_decisionsCount', valueColor: _kRed),
                    Container(height: 30, width: 1, color: Colors.grey.shade300),
                    _ProStat(label: '📜 Historial',   value: '${_history.length} jugadas', valueColor: Colors.deepPurple),
                  ],
                ),
              ),
              const SizedBox(height: 20),
              Text('Distribución de Puntos por Comensal',
                  style: GoogleFonts.outfit(fontSize: 15, fontWeight: FontWeight.bold, color: _kDark)),
              const SizedBox(height: 10),
              ..._players.map((player) {
                  ...
                  child: Column(
                    ...
                    children: [
                      Row(
                        ...
                        children: [
                          Row(children: [
                            Icon(player['icon'] as IconData, size: 14, color: playerColor),
                            const SizedBox(width: 6),
                            Text(player['name'] as String,
                                style: GoogleFonts.outfit(fontWeight: FontWeight.w600, fontSize: 13, color: _kDark)),
                          ]),
                          Text('$pts pts ($medals 🏆)',
              ...
              SizedBox(
                ...
                child: ElevatedButton(
                  ...
                  onPressed: () { Navigator.pop(ctx); _showBadgesModal(); },
                  child: Text('Ver Podio e Insignias',
              ...
              SizedBox(
                ...
                child: OutlinedButton(
                  ...
                  onPressed: () { Navigator.pop(ctx); _resetSessionScores(); },
                  child: Text('🔄 Reiniciar Sesión',
                      style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 14, color: Colors.red.shade600)),
                ),
              ),
              const SizedBox(height: 16),
            ],
  ...
  void _spinGame() {
    ...
    setState(() { _isSpinning = true; _selectedWinner = null; _currentChallenge = null; });
    ...
    Future<void> runSpin() async {
      try {
        for (int step = 0; step <= totalSteps; step++) {
          if (!mounted || _players.isEmpty) return;
          setState(() { _highlightedIndex = (_highlightedIndex + 1) % _players.length; });
          HapticFeedback.selectionClick();
          if (step < totalSteps) {
            await Future.delayed(Duration(milliseconds: 35 + (pow(step, 1.35) * 3).toInt()));
          }
        }
        ...
        setState(() {
          ...
          if (_selectedMode == 0) {
            ...
            _history.insert(0, {
              'winner': winnerName, 'detail': '🍽️ ¡Le toca elegir plato!',
              'time': TimeOfDay.now().format(context),
            });
          } else {
            ...
            _history.insert(0, {
              'winner': winnerName, 'detail': '🔥 Juicio Picante asignado',
              'time': TimeOfDay.now().format(context),
            });
          }
          if (_history.length > 5) _history.removeLast();
        ...
        debugPrintStack(stackTrace: st);
        if (!mounted) return;
        setState(() { _isSpinning = false; _selectedWinner = null; _currentChallenge = null; });
        _showFeedbackSnackbar('⚠️ No se pudo completar el giro. Inténtalo de nuevo.');
      }
    }
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Text('Zona Gamer de Palito',
        ...
        actions: [
          ...
          Container(
            ...
            decoration: BoxDecoration(color: _kYellow, borderRadius: BorderRadius.circular(12)),
            child: Row(children: [
              const Icon(Icons.local_fire_department_rounded, size: 15, color: Colors.black87),
              const SizedBox(width: 4),
              Text('$_decisionsCount',
                  style: GoogleFonts.outfit(fontWeight: FontWeight.bold, color: _kDark, fontSize: 13)),
            ]),
          ),
        ],
      ),
          ...
          child: Column(
            ...
            children: [
              ...
              _ModeSelector(
                ...
                onSelect: (m) { HapticFeedback.selectionClick(); setState(() => _selectedMode = m); },
              ),
              const SizedBox(height: 20),
              ...
              ScaleTransition(
                ...
                child: AnimatedContainer(
                  ...
                  decoration: BoxDecoration(
                    ...
                    boxShadow: [
                      BoxShadow(
                        color: (_selectedMode == 0 ? _kYellow : _kRed).withValues(alpha: 0.35),
                    ...
                    children: [
                        ...
                        child: ClipOval(
                          child: Image.asset(
                            ...
                            errorBuilder: (_, __, ___) => Container(
                              ...
                              child: const Icon(Icons.restaurant, color: _kDark),
                            ),
                          ),
                        ),
                      ),
                      ...
                      Text(
                        _selectedWinner == null
                            ? (_isSpinning
                                ? '⚡ Buscando comensal...'
                                : (_selectedMode == 0 ? '¡Gira para ver quién elige plato!' : '¡El juicio picante va a empezar!'))
                      ...
                      Text(
                        _selectedWinner != null
                            ...
                            : (_selectedMode == 0 ? 'Ruleta de Platos' : 'Juicio Pendiente'),
                        style: GoogleFonts.outfit(
                          ...
                          onTap: () {
                            if (_selectedWinner != null) {
                              _showChallengeOutcomeDialog(
                                _selectedWinner!['name'] as String, _currentChallenge!,
                              );
                            }
                          },
                          child: Container(
                            ...
                            child: Column(children: [
                              Text(_currentChallenge!,
                                  style: GoogleFonts.outfit(fontSize: 14, fontWeight: FontWeight.bold, color: _kDark),
                                  textAlign: TextAlign.center),
                              const SizedBox(height: 6),
                              Text('👆 Toca aquí para evaluar el reto',
                                  style: GoogleFonts.inter(fontSize: 11, fontWeight: FontWeight.bold, color: _kRed)),
                            ]),
                          ),
                        ),
                      ],
                ...
                child: SizedBox(
                  ...
                  child: ElevatedButton(
                    ...
                    child: Row(
                      ...
                      children: [
                        const Icon(Icons.casino_rounded, size: 22, color: _kYellow),
                        const SizedBox(width: 10),
                        Text(
                          _isSpinning
                              ? 'Girando Ruleta...'
                              : (_selectedMode == 0 ? '¡Girar Ruleta (Elegir Plato)!' : '¡Lanzar Juicio Picante!'),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Expanded(child: Text('Comensales en la Mesa (${_players.length})',
                      style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: _kDark))),
                  TextButton.icon(
                    onPressed: _showAddPlayerDialog,
                    icon: const Icon(Icons.add_rounded, size: 16, color: _kRed),
                    label: Text('Añadir', style: GoogleFonts.outfit(fontWeight: FontWeight.bold, color: _kRed, fontSize: 13)),
                  ),
                ],
              ),
              const SizedBox(height: 12),
                    ...
                    return Padding(
                      ...
                      child: GestureDetector(
                        ...
                        child: AnimatedContainer(
                          ...
                          decoration: BoxDecoration(
                            color: isHighlighted ? _kYellow.withValues(alpha: 0.3) : Colors.white,
                          ...
                          child: Column(
                            ...
                            children: [
                              ...
                              const SizedBox(height: 6),
                              Text(player['name'] as String,
                                  style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 12, color: _kDark),
              ...
              Row(
                mainAxisAlignment: MainAxisAlignment.spaceBetween,
                children: [
                  Text('Historial de la Sesión',
                      style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: _kDark)),
                  if (_history.isNotEmpty)
                    TextButton(
                      onPressed: _clearHistory,
                      child: Text('Limpiar',
                          style: GoogleFonts.inter(fontSize: 12, color: Colors.red.shade400, fontWeight: FontWeight.bold)),
                    ),
                ],
              ),
              const SizedBox(height: 12),
                ...
                ..._history.map((record) => Container(
                  ...
                  child: Row(
                    ...
                    children: [
                      ...
                      const SizedBox(width: 8),
                      Text(record['time']!,
                          style: GoogleFonts.inter(fontSize: 11, color: Colors.grey.shade400, fontWeight: FontWeight.bold)),
                    ],
                  ),
                )),
              const SizedBox(height: 24),
              ...
              SizedBox(
                ...
                child: OutlinedButton.icon(
                  ...
                  icon: const Icon(Icons.local_fire_department_rounded, color: _kRed, size: 18),
                  label: Text('Añadir Juicio Picante al Vuelo',
                      style: GoogleFonts.outfit(fontWeight: FontWeight.bold, fontSize: 13, color: _kRed)),
                ),
              ),
            ],
          ),
...
class _ModeSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Row(children: [
          _ModeTab(label: '🎯 Ruleta Pro',     index: 0, selectedMode: selectedMode, activeColor: _kYellow, onTap: onSelect),
          _ModeTab(label: '🔥 Juicio Picante', index: 1, selectedMode: selectedMode, activeColor: _kRed,    onTap: onSelect, activeTextColor: Colors.white),
        ]),
      );
}
...
class _ModeTab extends StatelessWidget {
  const _ModeTab({
    required this.label, required this.index, required this.selectedMode,
    required this.activeColor, required this.onTap, this.activeTextColor,
  });
  final String label;
  ...
  Widget build(BuildContext context) => Expanded(
        child: GestureDetector(
          ...
          child: AnimatedContainer(
            ...
            decoration: BoxDecoration(
              color: selectedMode == index ? activeColor : Colors.transparent,
              borderRadius: BorderRadius.circular(20),
            ),
...
class _BadgePill extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        child: Text(text,
            style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.bold, color: textColor ?? _kDark)),
      );
}
...
class _ProStat extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Column(children: [
        ...
        const SizedBox(height: 4),
        Text(value,
            style: GoogleFonts.outfit(fontSize: 18, fontWeight: FontWeight.bold, color: valueColor ?? _kDark)),
      ]);
}
