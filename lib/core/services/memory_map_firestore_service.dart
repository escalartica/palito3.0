...
class MemoryMapFirestoreService {
  ...
  CollectionReference<Map<String, dynamic>>? get _userMemoriesCollection {
  ...
  CollectionReference<Map<String, dynamic>>? get _userLocationsCollection {
    if (_currentUserId == null || groupId == null) {
      return null;
    }
  ...
  double? _parseDouble(dynamic value) {
    ...
    if (value is String) {
      ...
      final double? result = double.tryParse(normalized);
  ...
  bool _ensureAuthenticated({required String operation}) {
  ...
  void _logAuthState() {
    ...
    // El correo del usuario NUNCA debe aparecer en un log, y el uid entero
    // tampoco hace falta: con un prefijo basta para correlacionar dos líneas
    // del mismo log mientras se depura.
    _log(
      '👤 Sesión activa (${user.uid.substring(0, 4)}…), '
      'anónimo: ${user.isAnonymous}',
    );
  }
  ...
  Map<String, dynamic> _normalizeLocation(Map<String, dynamic> data) {
    final Map<String, dynamic> location = _parseMap(data['location']);

    // ------------------------------------------------------------------------
    // ADDRESS
    // ------------------------------------------------------------------------

    String address = _parseString(location['address']);
    ...
    if (address.isEmpty) {
      address = _parseString(data['locationAddress']);
    }
    ...
    double? lat = _parseDouble(location['lat']);
    ...
    double? lng = _parseDouble(location['lng']);
    ...
    if (!_areValidCoordinates(lat, lng)) {
      lat = null;
      lng = null;
    }
  ...
  Map<String, dynamic> _normalizeMemoryDocument(
    ...
    data['location'] = _normalizeLocation(data);

    // ------------------------------------------------------------------------
    // TITLE
    // ------------------------------------------------------------------------

    String title = _parseString(data['title']);

    if (title.isEmpty) {
      title = _parseString(data['restaurantName']);
    }

    if (title.isEmpty) {
      title = _parseString(data['name']);
    }
    ...
    String restaurantName = _parseString(data['restaurantName']);
    ...
    data['category'] = _parseString(data['category'], fallback: 'General');

    // ------------------------------------------------------------------------
    // RATING
    // ------------------------------------------------------------------------

    double? rating = _parseDouble(data['rating'] ?? data['score']);
    ...
    data['wouldReturn'] = _parseBool(
      data['wouldReturn'] ?? data['would_return'],
    );
    ...
    data['imageUrls'] = _parseStringList(
      data['imageUrls'] ?? data['images'] ?? data['image_paths'],
    );
    ...
    final String videoUrl = _parseString(
      data['videoUrl'] ?? data['video'] ?? data['video_url'],
    );

    data['videoUrl'] = videoUrl.isEmpty ? null : videoUrl;
    ...
    data['specificFields'] = _parseMap(
      data['specificFields'] ?? data['specific_fields'],
    );
  ...
  MemoryModel _memoryModelFromFirestoreDocument(
    ...
    final Map<String, dynamic> rawData = document.data() ?? <String, dynamic>{};

    final Map<String, dynamic> normalizedData = _normalizeMemoryDocument(
      document.id,
      rawData,
    );

    return MemoryModel.fromMap(normalizedData);
  }
  ...
  Future<MemoryModel?> getMemoryById(String memoryId) async {
    if (!_ensureAuthenticated(operation: 'getMemoryById')) {
      return null;
    }

    final String normalizedMemoryId = memoryId.trim();
    ...
    try {
      final DocumentSnapshot<Map<String, dynamic>> document = await collection
      ...
      return _memoryModelFromFirestoreDocument(document);
    } catch (e, stack) {
      _log(
  ...
  Stream<List<Map<String, dynamic>>> getMemoriesStream() {
    ...
    _log('🗺️ Iniciando stream Firestore:');

    _log('📁 groups/$groupId/$_memoriesCollection');
    ...
    return collection.snapshots().map((
      ...
      _log('🗺️ FIRESTORE SNAPSHOT DE MEMORIAS');
      ...
      for (final QueryDocumentSnapshot<Map<String, dynamic>> doc
        ...
        try {
          ...
          final Map<String, dynamic> data = _normalizeMemoryDocument(
            doc.id,
            rawData,
          );
          ...
          final String title = _parseString(
            data['title'],
            fallback: '(sin título)',
          );

          final String address = _parseString(
            location['address'],
  ...
  Stream<List<MemoryModel>> getMemoryModelsStream() {
    ...
    if (collection == null) {
      ...
      return Stream.value(<MemoryModel>[]);
    }

    return collection.snapshots().map((
      QuerySnapshot<Map<String, dynamic>> snapshot,
    ) {
      final List<MemoryModel> result = <MemoryModel>[];

      for (final QueryDocumentSnapshot<Map<String, dynamic>> doc
          in snapshot.docs) {
        try {
          result.add(_memoryModelFromFirestoreDocument(doc));
        } catch (e, stack) {
          _log(
  ...
  Future<void> updateMemoryCoordinates({
    ...
    if (!_ensureAuthenticated(operation: 'updateMemoryCoordinates')) {
      // Antes esto era un `return` mudo: la operación se completaba "con
      // éxito" sin haber escrito nada. Pérdida de datos invisible.
      throw StateError(
        'Tu sesión ha caducado. Vuelve a iniciar sesión para guardar.',
      );
    }
    ...
    final CollectionReference<Map<String, dynamic>>? memoriesCollection =
        _userMemoriesCollection;
    ...
    await memoriesCollection.doc(normalizedMemoryId).update(<String, dynamic>{
      'location.lat': lat,
      'location.lng': lng,
    });
  }
  ...
  Future<void> saveMemory({
    ...
    if (!_ensureAuthenticated(operation: 'saveMemory')) {
      // Antes esto era un `return` mudo: la operación se completaba "con
      // éxito" sin haber escrito nada. Pérdida de datos invisible.
      throw StateError(
        'Tu sesión ha caducado. Vuelve a iniciar sesión para guardar.',
    ...
    if (normalizedMemoryId.isEmpty) {
      throw ArgumentError('memoryId no puede estar vacío.');
    }
    ...
    try {
      ...
      if (memoriesCollection == null) {
        throw StateError('No se pudo obtener la colección de memorias.');
      }

      if (locationsCollection == null) {
        throw StateError('No se pudo obtener la colección de localizaciones.');
      }
      ...
      final DocumentReference<Map<String, dynamic>> memoryReference =
          memoriesCollection.doc(normalizedMemoryId);

      final DocumentReference<Map<String, dynamic>> locationReference =
          locationsCollection.doc(normalizedMemoryId);
      ...
      await _firestore.runTransaction((Transaction transaction) async {
        ...
        final DocumentSnapshot<Map<String, dynamic>> existingMemory =
            await transaction.get(memoryReference);

        final DocumentSnapshot<Map<String, dynamic>> existingLocation =
            await transaction.get(locationReference);

        // ======================================================================
        // COPIA DE DATOS
        // ======================================================================

        final Map<String, dynamic> data = Map<String, dynamic>.from(memoryData);

        // ======================================================================
        // ID
        // ======================================================================

        data['id'] = normalizedMemoryId;

        // ======================================================================
        // LOCATION
        // ======================================================================

        final Map<String, dynamic> location = _normalizeLocation(data);
        ...
        final Timestamp? parsedDate = _parseTimestamp(data['date']);

        if (parsedDate != null) {
          data['date'] = parsedDate;
        } else if (existingMemory.exists) {
          final dynamic existingDate = existingMemory.data()?['date'];

          if (_hasUsableValue(existingDate)) {
        ...
        transaction.set(memoryReference, data, SetOptions(merge: true));
        ...
        _log('✅ Memoria guardada correctamente.');

        _log('   ID: $normalizedMemoryId');

        _log('   Usuario: $_currentUserId');

        _log('   Título: $title');

        _log('   Restaurante: $restaurantName');

        _log('   Categoría: ${data['category']}');
        ...
        if (_areValidCoordinates(lat, lng)) {
          _log('📍 Coordenadas guardadas correctamente:');

          _log('   Lat: $lat');

          _log('   Lng: $lng');
        } else {
          _log('⚠️ Memoria guardada sin coordenadas válidas.');

          _log('   Location: $location');
        }

        _log('📍 Localización secundaria sincronizada.');
      });
    } catch (e, stack) {
      _log(
  ...
  Future<void> deleteMemory(String memoryId) async {
    if (!_ensureAuthenticated(operation: 'deleteMemory')) {
      // Antes esto era un `return` mudo: la operación se completaba "con
      // éxito" sin haber escrito nada. Pérdida de datos invisible.
      throw StateError(
        'Tu sesión ha caducado. Vuelve a iniciar sesión para guardar.',
    ...
    try {
      ...
      batch.delete(memoriesCollection.doc(normalizedMemoryId));

      batch.delete(locationsCollection.doc(normalizedMemoryId));

      await batch.commit();

      _log('🗑️ Memoria eliminada correctamente:');

      _log('   ID: $normalizedMemoryId');

      _log('   Usuario: $_currentUserId');
    } catch (e, stack) {
      _log('❌ Error al eliminar recuerdo: $e');
  ...
  Stream<List<Map<String, dynamic>>> getLocationsStream() {
    ...
    _log('📍 Iniciando stream de localizaciones:');

    _log('📁 groups/$groupId/$_locationsCollection');
    ...
    return collection.snapshots().map((
      ...
      for (final QueryDocumentSnapshot<Map<String, dynamic>> doc
        ...
        try {
          ...
          final Map<String, dynamic> normalizedLocation = _normalizeLocation(
            rawData,
          );

          final double? lat = _parseDouble(normalizedLocation['lat']);

          final double? lng = _parseDouble(normalizedLocation['lng']);
  ...
  Future<void> saveLocationFromMemory(MemoryModel memory) async {
    final String locationId = memory.id.trim();

    if (locationId.isEmpty) {
      throw ArgumentError('No se puede guardar una localización sin ID.');
    }

    await saveLocation(
      locationId: locationId,
      locationData: memory.location.toMap(),
    );
  }
  ...
  Future<void> saveLocation({
    ...
    if (!_ensureAuthenticated(operation: 'saveLocation')) {
      // Antes esto era un `return` mudo: la operación se completaba "con
      // éxito" sin haber escrito nada. Pérdida de datos invisible.
      throw StateError(
        'Tu sesión ha caducado. Vuelve a iniciar sesión para guardar.',
    ...
    final String normalizedLocationId = locationId.trim();
    ...
    try {
      ...
      final Map<String, dynamic> data = Map<String, dynamic>.from(locationData);

      // ======================================================================
      // NORMALIZAR
      // ======================================================================

      final Map<String, dynamic> normalizedLocation = _normalizeLocation(data);

      data['id'] = normalizedLocationId;

      data['address'] = normalizedLocation['address'];

      data['lat'] = normalizedLocation['lat'];

      data['lng'] = normalizedLocation['lng'];
      ...
      _log('📍 Localización guardada correctamente.');

      _log('   ID: $normalizedLocationId');

      _log('   Usuario: $_currentUserId');

      final double? lat = _parseDouble(data['lat']);

      final double? lng = _parseDouble(data['lng']);
  ...
  Future<void> deleteLocation(String locationId) async {
    if (!_ensureAuthenticated(operation: 'deleteLocation')) {
      // Antes esto era un `return` mudo: la operación se completaba "con
      // éxito" sin haber escrito nada. Pérdida de datos invisible.
      throw StateError(
        'Tu sesión ha caducado. Vuelve a iniciar sesión para guardar.',
    ...
    try {
      ...
      await collection.doc(normalizedLocationId).delete();

      _log('🗑️ Localización secundaria eliminada:');

      _log('   ID: $normalizedLocationId');
    } catch (e, stack) {
      _log('❌ Error al eliminar localización: $e');
